...
public class Main {
    public static void main(String[] args) {
        ...
        long[] fishAtCounter = new long[9];
        String[] parts = input.split(",");
        for (String part : parts) {
            fishAtCounter[parseInt(part)]++;
        }
        
        for (int i = 0; i < 256; i++) {
            long zeroes = fishAtCounter[0];
            for (int j = 0; j < 8; j++) {
                fishAtCounter[j] = fishAtCounter[j+1];
            }
            fishAtCounter[8] = zeroes;
            fishAtCounter[6] += zeroes;
        }

        for (long l : fishAtCounter) {
            res += l;
        }
